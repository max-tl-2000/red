...
import { getPropertyPartySettingsToExport } from '../../../dal/screeningCriteriaRepo';
...
export const exportPropertyPartySettings = async (ctx, { propertyIdsToExport, columnHeaders: columnHeadersOrdered }) => {
  const { foreignKeys } = spreadsheet.PropertyPartySettings;
  const columnHeaders = getColumnHeaders(spreadsheet.PropertyPartySettings.columns);
  ...
  const propertyPartySettings = await getPropertyPartySettingsToExport(ctx, dbSimpleFields, propertyIdsToExport);

  const columnHeadersOrderedMapped = getColumnHeadersMappedWithDB(columnHeadersOrdered, DB_MAPPERS);
  return buildDataPumpFormat(propertyPartySettings, columnHeadersOrderedMapped);
};
