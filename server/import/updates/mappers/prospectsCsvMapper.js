...
const SKIP_VALUES = ['ResProspects', 'Prospect_Code'];

const INCLUDED_STATUS = ['1', '2', '6'];

const isValid = row => !SKIP_VALUES.includes(row.Prospect_Code) && INCLUDED_STATUS.includes(row.Status);
...
const MAPPING = [
  {
    excel: 'prospectId',
    fn: row => (!row.Parent_Code ? row.Prospect_Code : ''),
  },
  {
    excel: 'externalId',
    fn: row => (!row.Parent_Code ? row.Tenant_Code : ''),
  },
  {
    excel: 'fullName',
    fn: row => {
  ...
  {
    excel: 'roommateId',
    fn: row => (row.Parent_Code ? row.Roommate_Code : ''),
  },
  { csv: 'Parent_Code', excel: 'member' },
  { csv: 'Date_Applied', excel: 'dateApplied' },
  { csv: 'Prospect_Code', excel: 'prospectCode' },
...
export const prospectsCsvMapper = row => ({
  valid: isValid(row),
