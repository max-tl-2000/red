...
const formatUrl = (path, isAuthCall = false) => {
  const adjustedPath = path[0] !== '/' ? `/${path}` : path;
  // Prepend `/api` to relative URL, to proxy to API server.
  return isAuthCall ? `${adjustedPath}` : `/api${adjustedPath}`;
};
...
['get', 'post', 'put', 'patch', 'del'].reduce((acc, method) => {
  acc[method] = (path, options) => {
    ...
    const isAuthCall = options && options.isAuthCall;
    const request = superagent[method](formatUrl(path, isAuthCall));
