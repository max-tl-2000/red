...
['get', 'post', 'put', 'patch', 'del'].reduce((acc, method) => {
  acc[method] = (path, methodArgs) => {
    const { noPrefix, ...options } = methodArgs || {};
    const requestPromise = deferred();

    const request = superagent[method](noPrefix ? path : formatUrl(path));

    if (options && options.params) request.query(options.params);

    if (acc.headers) request.set(acc.headers);

    if (options && options.data) request.send(options.data);

    if (options && options.timeout) request.timeout(options.timeout);
