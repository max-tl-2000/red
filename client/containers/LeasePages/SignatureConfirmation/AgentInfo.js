...
import { cf, g } from './AgentInfo.scss';
...
export const AgentInfo = ({ agent }) => (
  <div className={cf('agent-info')}>
    ...
    <div className={cf('contact-info')}>
      ...
      <div>
        <T.Caption secondary>{agent.title}</T.Caption>
        <T.Caption>{formatPhoneNumber(agent.phone)}</T.Caption>
        <T.Caption>{agent.email}</T.Caption>
      </div>
    </div>
  </div>
);
