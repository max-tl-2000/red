  ...
  return (
    <DemoSection key={name} title={name}>
      ...
      <PrettyPrint>
        {`
             <div style={ { padding: 20 } }>
               <div>
                 <${name}><Link href="about:blank" rel="noreferrer noopener" target="_blank">This is a Hyperlink using "{ name }"</Link></${name}>
               </div>
               <div>
      ...
      <div style={{ padding: 20 }}>
        <div>
          <Ele>
            <Link href="about:blank" rel="noreferrer noopener" target="_blank">
              This is a Hyperlink using "{name}"
            </Link>
          </Ele>
        </div>
        <div>
      ...
      <div>
        <SubHeader>Handling different types of children</SubHeader>
        <Ele>
          <span>This is an element with a p inside</span>
        </Ele>
        <Ele>The following element receive a null value: </Ele>
        <Ele>{null}</Ele>
        <Ele>{[<span key="1">An array of</span>, <span key="2">elements</span>]}</Ele>
        <Ele>{[1, 2, 3, 'some srtring here', {}, null, undefined]}</Ele>
      </div>
    </DemoSection>
  );
};
...
export default class TypographyDemo extends Component {
  ...
  render() {
    return (
      <DemoPage title="Typography">
        <DemoSection title="What are the Typography elements?">
          <MDBlock>{`
                 Typography elements are the ones that should be used whenever we need to render some text in the app.

                 The benefits from using them are:
                 - Text will be consistent all across the app
                 - Common behavior like truncation/ellipsis can be implemented as part of the component
                 - Css styles can be managed by the components and changing styles can be implemented using flags
               `}</MDBlock>
        </DemoSection>
